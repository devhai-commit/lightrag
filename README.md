...
#### Bước 3: Khởi động Backend lần đầu (tự động tạo bảng qua `AUTO_CREATE_TABLES`)
...
Đợi log hiện `Application startup complete` rồi **để backend chạy tiếp** (mở terminal mới cho bước sau). Bảng `documents`, `document_versions`, ... chỉ được tạo ở bước này — chạy `alembic upgrade head` trước khi backend khởi động lần đầu sẽ báo lỗi `relation "documents" does not exist`.

#### Bước 4: Chạy Database Migrations (RLS policies, v.v.)
...
- **Backend**: đã chạy từ Bước 3 (`uvicorn app.main:app --reload --port 8000`)
...
- **Backend**: đã chạy từ Bước 3 (`./run_bk.sh`)
- **Frontend**: `cd ../micco-frontend && npm run dev`
- **Truy cập**: http://localhost:5174
...
| `LLM_PROVIDER` | Provider: `gemini`, `ollama` hoặc `deepseek` | `gemini` |
| `LLM_MODEL_FAST` | Model LLM sử dụng (Gemini) | `gemini-2.5-flash` |
...
| `DEEPSEEK_MODEL` | Model DeepSeek (`deepseek-chat` hoặc `deepseek-reasoner`) | `deepseek-chat` |
...
| `CHROMA_HOST` | ChromaDB host | `localhost` |
| `CHROMA_PORT` | ChromaDB port | `8003` |
| `LLM_PROVIDER` | Provider: `gemini`, `ollama`, or `deepseek` | `gemini` |
| `LLM_MODEL_FAST` | LLM model to use (Gemini) | `gemini-2.5-flash` |

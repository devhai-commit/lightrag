...
// ─── Reasoning Timeline (persisted, collapsible — survives streaming + reload) ─
function ReasoningTimeline({ steps }) {
    ...
    return (
        <div className="w-full mb-1.5">
            <details className="group border border-gray-200 dark:border-gray-800 rounded-xl bg-white dark:bg-gray-800/50 overflow-hidden shadow-sm">
                <summary className="flex items-center justify-between px-3 py-2 cursor-pointer bg-violet-50/50 dark:bg-gray-800 hover:bg-violet-50 dark:hover:bg-gray-750 transition-colors select-none">
                    <div className="flex items-center gap-2">
                        <Brain className="w-4 h-4 text-violet-500" />
                        <span className="text-xs font-semibold text-violet-800 dark:text-violet-400">
                            Quá trình suy luận ({steps.length} bước)
                        </span>
                    </div>
                    <ChevronDown className="w-4 h-4 text-gray-400 group-open:rotate-180 transition-transform" />
                </summary>
                <div className="p-3 space-y-2 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
                    {steps.map((s, i) => {
                        const cfg = STEP_CONFIG[s.step] || STEP_CONFIG.analyzing;
                        const Icon = cfg.icon;
                        return (
                            <div key={s.id || i} className="flex items-start gap-2.5 text-xs">
                                ...
                                <div className="flex-1 min-w-0 pt-0.5">
                                    <p className="font-medium text-gray-700 dark:text-gray-300">{cfg.label}</p>
                                    {s.detail && <p className="text-gray-400 dark:text-gray-500 mt-0.5 truncate">{s.detail}</p>}
                                    {s.step === 'sources_found' && s.sourceBadges?.length > 0 && (
                                        <div className="flex flex-wrap gap-1 mt-1">
                                            {s.sourceBadges.map((b, bi) => (
                                                <span key={bi} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800">
                                                    [{b}]
                                                </span>
                                            ))}
                                        </div>
...
// ─── Chat Message ─────────────────────────────────────────────────────────────
function ChatMessage({ msg, onSourceClick, onViewGraphPath }) {
    ...
    const reasoningSteps = msg.reasoningSteps || [];
    ...
    return (
        <div className={`flex gap-3 group/msg ${isUser ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-[82%] min-w-0 space-y-1 ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
                ...
                {/* Reasoning steps — persisted timeline once the answer is done */}
                {!isUser && !isStreaming && <ReasoningTimeline steps={reasoningSteps} />}
                ...
                {!isUser && relatedEntities.length > 0 && !isStreaming && !isFallback && (
                    <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        {onViewGraphPath && (
                            <button
                                onClick={() => onViewGraphPath(relatedEntities.map(e => typeof e === 'string' ? e : (e.name || '')).filter(Boolean))}
                                className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-violet-50 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-800 text-violet-700 dark:text-violet-400 hover:bg-violet-100 dark:hover:bg-violet-500/20 transition-colors"
                                title="Xem đường đi suy luận trên đồ thị tri thức"
                            >
                                <ExternalLink className="w-3 h-3" />
                                Xem đường đi trên đồ thị
                            </button>
                        )}
                    </div>
                )}
...
export default function ChatAssistant() {
    ...
    const handleMicClick = useCallback(async () => {
        ...
        try {
            ...
            recorder.onstop = async () => {
                ...
                if (blob.size < 1000) {
                    setMicError('Không phát hiện giọng nói. Vui lòng thử lại.');
                ...
                try {
                    ...
                    if (data.text) {
                        ...
                        setMicError('Không nhận diện được nội dung.');
                    }
                } catch (err) {
                    setMicError(err.message || 'Lỗi kết nối khi chuyển giọng nói thành văn bản.');
                } finally {
                    setRecordingState('idle');
                }
            };
            ...
            setMicError('Không thể truy cập micro. Vui lòng cấp quyền micro cho trình duyệt.');
            setRecordingState('idle');
        }
    }, [recordingState]);
    ...
    const handleSend = useCallback(async () => {
        ...
        let finalEntities = [];
        let stepCounter = 0;
        const accSteps = [{ id: 'step-0', step: 'analyzing', detail: '' }];

        const updateMsg = (patch) => {
            setMessages(prev => prev.map(m => m.id === aiId ? { ...m, ...patch } : m));
        };

        const pushStep = (step, detail, extra = {}) => {
            stepCounter += 1;
            accSteps.push({ id: `step-${stepCounter}`, step, detail, ...extra });
        };
            ...
            await readSSEStream(res, {
                onChunk: (chunk) => {
                    ...
                    if (evtType === 'status') {
                        ...
                        const step = chunk.step || chunk.data?.step;
                        const detail = chunk.detail || chunk.data?.detail || '';
                        updateMsg({ reasoningStep: step, reasoningDetail: detail });
                        pushStep(step, detail);
                        ...
                        const badges = [...new Set(finalSources.map(s => s.index).filter(Boolean))].slice(0, 6);
                        pushStep('sources_found', `Đã tìm thấy ${finalSources.length} nguồn`, {
                ...
                onDone: () => {
                    pushStep('done', 'Hoàn thành');
                    setMessages(prev => prev.map(m => m.id === aiId
                        ? {
                            ...m,
                            streaming: false,
                            reasoningStep: null,
                            reasoningSteps: accSteps,
                            sources: finalSources,
                            related_entities: finalEntities,
                            thinking: accThinking || null,
                ...
                onError: (err) => {
                    ...
                    setMessages(prev => prev.map(m => m.id === aiId
                        ? { ...m, streaming: false, reasoningStep: null, reasoningSteps: accSteps, content: accContent || '⚠️ Có lỗi xảy ra. Vui lòng thử lại.', sources: [] }
                        : m
                    ));
                    setSending(false);
                },
    ...
    // ─── View reasoning path on Knowledge Graph ───────────────────────────────
    const handleViewGraphPath = useCallback((entities) => {
        if (!selectedWs || !entities || entities.length === 0) return;
        const params = new URLSearchParams({
            ws: String(selectedWs.id),
            entities: entities.join(','),
        });
        navigate(`/graph-knowledge?${params.toString()}`);
    }, [selectedWs, navigate]);
                    ...
                    <div className="space-y-4">
                        {messages.map(msg => (
                            <div key={msg.id} className="animate-fade-in-up">
                                <ChatMessage msg={msg} onSourceClick={handleSourceClick} onViewGraphPath={handleViewGraphPath} />
                            </div>
                        ))}
                    </div>
            ...
            {selectedWs && (
                <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-800 p-3 lg:p-4 flex-shrink-0 rounded-b-2xl">
                    <div className="max-w-4xl mx-auto w-full">
                        ...
                        {micError && (
                            <div className="mb-2 px-1 text-[11px] text-red-500">{micError}</div>
                        )}
                        ...
                        <div className="flex items-end gap-2 relative group">
                            ...
                            <button
                                ...
                                title={recordingState === 'recording' ? 'Dừng ghi âm' : 'Ghi âm giọng nói'}

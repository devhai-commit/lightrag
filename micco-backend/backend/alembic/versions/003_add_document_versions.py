...
def upgrade() -> None:
    ...
    Idempotent: `document_versions` may already exist if the app's
    AUTO_CREATE_TABLES startup ran before this migration (it's a mapped
    ORM model), so skip creation when the table is already present.
    """
    bind = op.get_bind()
    insp = inspect(bind)

    if "document_versions" not in insp.get_table_names():
        op.create_table(
            "document_versions",
    ...
    # Enable RLS for document_versions (idempotent in Postgres)
    op.execute("ALTER TABLE document_versions ENABLE ROW LEVEL SECURITY")

    # Admin bypass
    op.execute("DROP POLICY IF EXISTS docver_admin_policy ON document_versions")
    op.execute("""

...
def upgrade() -> None:
    # Idempotent: visibility/owner_id are now part of the current
    # KnowledgeBase ORM model, so AUTO_CREATE_TABLES may have already
    # created them on a fresh DB — only add what's actually missing.
    bind = op.get_bind()
    insp = inspect(bind)
    existing_columns = {c["name"] for c in insp.get_columns("knowledge_bases")}
    ...
    existing_indexes = {ix["name"] for ix in insp.get_indexes("knowledge_bases")}
    if "ix_knowledge_bases_owner_id" not in existing_indexes:
        op.create_index(

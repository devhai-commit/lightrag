...
# Find .env file - check backend root first, fallback for Docker
_candidate = Path(__file__).resolve().parent.parent.parent / ".env"
ENV_FILE = str(_candidate) if _candidate.exists() else ".env"

# Docling defaults to torch.compile()-ing its layout/table-detection models,
# which requires a C++ compiler (cl.exe on Windows). Most dev machines don't
# have MSVC Build Tools installed, so document parsing fails with
# "InvalidCxxCompiler: Compiler: cl is not found". Disable compilation by
# default; set DOCLING_INFERENCE_COMPILE_TORCH_MODELS=true in the real
# environment to re-enable it on machines that do have a C++ toolchain.
...
class Settings(BaseSettings):
    ...
    # LLM Provider: "gemini" | "ollama" | "deepseek"
    ...
    # STT Provider (Speech-to-Text): "phowhisper"
    STT_PROVIDER: str = Field(default="phowhisper")
    STT_MODEL: str = Field(default="vinai/PhoWhisper-medium")
    # "auto" (CUDA if available else CPU) | "cuda" | "cpu"
    STT_DEVICE: str = Field(default="auto")
    # Reject uploads longer than this many seconds (protects server from huge inference jobs)
    STT_MAX_AUDIO_SECONDS: int = Field(default=60)
    # Reject uploads larger than this many bytes (raw container size, pre-decode)
    STT_MAX_FILE_SIZE_MB: int = Field(default=15)

    # ChromaDB
    CHROMA_HOST: str = Field(default="localhost")
    CHROMA_PORT: int = Field(default=8003)
